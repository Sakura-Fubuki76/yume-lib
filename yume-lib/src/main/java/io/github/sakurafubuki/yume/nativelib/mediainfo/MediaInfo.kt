...
data class MediaInfo(
    ...
    fun getFrame(durationMillis: Long = -1): Bitmap? {
        ...
        return if (result == true) bitmap.rotateIfNeeded(videoStream.rotation) else null
    }
    ...
    fun getFrameAt(durationMillis: Long = -1): Bitmap? {
        if (videoStream == null) return null
        return frameLoader?.getFrame(durationMillis)?.rotateIfNeeded(videoStream.rotation)
    }

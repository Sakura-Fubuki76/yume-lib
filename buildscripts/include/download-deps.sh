#!/bin/bash -e

. ./include/depinfo.sh

[ -z "$IN_CI" ] && IN_CI=0
[ -z "$WGET" ] && WGET=wget

mkdir -p deps && cd deps

# mbedtls
if [ ! -d mbedtls ]; then
	mkdir mbedtls
	$WGET https://github.com/Mbed-TLS/mbedtls/releases/download/mbedtls-$v_mbedtls/mbedtls-$v_mbedtls.tar.bz2 -O - | \
		tar -xj -C mbedtls --strip-components=1
fi

# dav1d
[ ! -d dav1d ] && git clone https://github.com/videolan/dav1d

# ffmpeg
if [ ! -d ffmpeg ]; then
	args=()
	[ $IN_CI -eq 1 ] && args+=(--depth=1 -b "$v_ci_ffmpeg")
	git clone https://github.com/FFmpeg/FFmpeg ffmpeg "${args[@]}"
fi

# freetype2
[ ! -d freetype2 ] && git clone --depth=1 --recurse-submodules https://github.com/freetype/freetype.git freetype2 -b VER-${v_freetype//./-}

# fribidi
if [ ! -d fribidi ]; then
	mkdir fribidi
	$WGET https://github.com/fribidi/fribidi/releases/download/v$v_fribidi/fribidi-$v_fribidi.tar.xz -O - | \
		tar -xJ -C fribidi --strip-components=1
fi

# harfbuzz
if [ ! -d harfbuzz ]; then
	mkdir harfbuzz
	$WGET https://github.com/harfbuzz/harfbuzz/releases/download/$v_harfbuzz/harfbuzz-$v_harfbuzz.tar.xz -O - | \
		tar -xJ -C harfbuzz --strip-components=1
fi

# unibreak
if [ ! -d unibreak ]; then
	mkdir unibreak
	$WGET https://github.com/adah1972/libunibreak/releases/download/libunibreak_${v_unibreak//./_}/libunibreak-${v_unibreak}.tar.gz -O - | \
		tar -xz -C unibreak --strip-components=1
fi

# libxml2
[ ! -d libxml2 ] && git clone --depth=1 https://gitlab.gnome.org/GNOME/libxml2.git libxml2 -b v${v_libxml2}

# fontconfig
[ ! -d fontconfig ] && git clone --depth=1 https://gitlab.freedesktop.org/fontconfig/fontconfig.git fontconfig -b ${v_fontconfig}

# libass
[ ! -d libass ] && git clone https://github.com/libass/libass

# libyuv
mkdir -p libyuv
[ ! -d libyuv/.git ] && git clone --depth=1 https://chromium.googlesource.com/libyuv/libyuv libyuv -b $v_libyuv

# soundtouch
mkdir -p soundtouch
[ ! -d soundtouch/.git ] && git clone --depth=1 https://codeberg.org/soundtouch/soundtouch soundtouch -b $v_soundtouch

cd ..
